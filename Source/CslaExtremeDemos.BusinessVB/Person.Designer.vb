...
#Region " Business Properties "
    ...
    ''' </summary>
    Public Shared ReadOnly CivilStateProperty As PropertyInfo(Of Byte) = RegisterProperty(Of Byte)(Function(p) p.CivilState, "Civil State")

    ''' <summary>
    ''' Gets or sets the Civil State.
    ''' </summary>
    ''' <value>The Civil State.</value>
    Public Property CivilState As CivilStates
        Get
            Return GetPropertyConvert(Of Byte, CivilStates)(CivilStateProperty)
        End Get
        Set
            SetPropertyConvert(Of Byte, CivilStates)(CivilStateProperty, Value)
        End Set
    End Property
...
#Region " Data Access "
    ...
    Private Sub Fetch(dr As SafeDataReader)
        ...
        'LoadProperty(DeptIdProperty, CType(dr.GetValue("DeptId"), Short?))
    ...
    Protected Overrides Sub DataPortal_Insert()
        Using ctx = TransactionManager(Of SqlConnection, SqlTransaction).GetManager(Database.CslaExtremeDemosDatabaseConnection, False)
            Using cmd = New SqlCommand("dbo.AddPerson", ctx.Connection)
                ...
                cmd.ExecuteNonQuery()
                OnInsertPost(args)
                LoadProperty(PersonIdProperty, DirectCast(cmd.Parameters("@PersonId").Value, Int32))
            End Using
            ctx.Commit()
        End Using
    End Sub
